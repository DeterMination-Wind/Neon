...
- 同步并入最新 `Radial Build Menu`：新增鼠标旁常驻 HUD 预览，平时可见、触发时更稳。
- 按住圆盘快捷键时 HUD 会固定在当前位置，便于精确选择槽位；松开后继续跟随鼠标。
- 新增常驻状态透明度调节，并完成 Neon 内整合同步与版本升级。
...
- Merged the latest `Radial Build Menu` update: an optional always-visible HUD preview near the cursor is now included in Neon.
...
- Added idle preview opacity control and synced the bundled module update into this Neon release.

### Previous Update (v7.0.1)
