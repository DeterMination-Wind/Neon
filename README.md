...
- 修复 MindustryX 端 OverlayUI 注册卡在 pending 队列的问题，窗口现在会在原生 `OverlayUI` 就绪后立即回放并显示。
- 修复旧配置里 `overlayUI.*` 状态类型不一致可能导致的启动闪退，减少 X 端因为遗留设置崩溃的情况。
- 原版客户端继续使用 Neon 内置 overlay 兼容层；MindustryX 端继续走原生 OverlayUI，不再要求额外桥接模组。
...
- Fixed the MindustryX OverlayUI registration path so pending windows replay as soon as the native `OverlayUI` becomes available.
- Fixed crashes caused by mismatched `overlayUI.*` state types in older configs, reducing startup failures on X builds.
- Vanilla clients still use Neon's built-in overlay compatibility layer, while MindustryX clients continue using the native OverlayUI path directly.
