...
- Neon 现在内置 OverlayUI 兼容层；在非 MindustryX 客户端中，支持 Overlay 的模块也能直接显示统一的悬浮窗口管理器。
- Overlay 齿轮按钮固定在屏幕左侧中部，更容易发现；按 `Z` 也可以直接打开或关闭 Overlay 编辑器。
...
- Neon now includes a built-in OverlayUI compatibility layer, so overlay-capable modules can use a shared floating window manager even on non-MindustryX clients.
