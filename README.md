...
Neon 是一个 Mindustry 纯客户端工具集（Java 模组），将以下 16 个模组合并为一个安装包：
...
如果你只想安装一次、一次性获得这十六类常用“信息叠加 + 操作效率 + 数据查询 + 自动更新 + 蓝图发布订阅 + 反向逻辑排查 + 数据补丁对比”功能，Neon 会更省事。

提示：Neon 已包含上述 16 个模块的功能，建议不要与它们的独立版本同时启用，避免重复功能或 UI 冲突。

### 本次更新（v7.0.1）

- 修复部分 MindustryX 环境导入 `Neon-v7.0.0` 闪退：`NoClassDefFoundError: Could not initialize class mindustryX.features.ui.OverlayUI`。
- 调整 `mdtxcompat` 桥接实现为懒加载与无初始化探测（`Class.forName(..., false, ...)`），避免在模组加载期提前触发 MindustryX UI 类初始化。
- 发布工作流改为上传带版本号文件名的资产（`Neon-v<tag>.jar/.zip`），避免不同版本资产名冲突。
...
Neon is a client-side Mindustry toolkit (Java mod) that bundles sixteen modules into one package:
...
Note: Neon already contains these sixteen modules. Do not enable standalone versions at the same time, or you may get duplicate UI and feature conflicts.

### What's New (v7.0.1)

- Fixed startup crashes on some MindustryX environments when importing `Neon-v7.0.0`: `NoClassDefFoundError: Could not initialize class mindustryX.features.ui.OverlayUI`.
- Reworked `mdtxcompat` bridges to lazy, non-initializing symbol resolution (`Class.forName(..., false, ...)`) to avoid triggering MindustryX UI class initialization during mod load.
- Updated release workflow to publish versioned asset names (`Neon-v<tag>.jar/.zip`) to prevent ambiguous artifacts across versions.
...
3) Configure all modules in `Settings -> Mods -> Neon`, including WhoUsesThisBuilding and PatchViewer groups.
