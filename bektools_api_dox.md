# BEK-Tools 接口说明（API dox）

本文件描述 BEK-Tools 对外提供的“接口”：设置入口、子模组聚合方式、以及对整合包维护者有用的内部约定（`bekBundled`、`bekBuildSettings` 等）。

## 1. 玩家接口

### 1.1 设置入口

BEK-Tools 在设置中新增一个一级分类（见 `bektools/BekToolsMod.java`）：
...
热键与功能全部来自三子模组本身；BEK-Tools 不会重定义它们，只做“打包与聚合设置”。
...
BEK-Tools 使用该方法把三个子模组的设置内容插入到自己的一级分类里。
...
各子模组对 MindustryX 的可选集成保持原样（通常使用反射以避免未安装 MindustryX 时崩溃）。BEK-Tools 不统一管理 OverlayUI 窗口，只负责把代码打包在一起。
