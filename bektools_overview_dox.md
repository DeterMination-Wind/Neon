# BEK-Tools 文档（维护者/开发者）

本目录下的 `*_dox.md` 为 BEK-Tools 仓库级说明文档，统一放在仓库根目录，便于维护与发布。

## 1. 这个 Mod 做什么

BEK-Tools 是一个 **客户端工具整合包**（JavaMod），把以下 3 个子模组的源码合并在一个包里发布：
...
- 主类：`src/main/java/bektools/BekToolsMod.java`
  ...
  - 实例化三个子模组
  - 将三个子模组的设置页面合并到 BEK-Tools 的一级分类下
