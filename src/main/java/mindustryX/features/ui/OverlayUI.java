...
/**
 * Java compatibility implementation of the MindustryX OverlayUI API.
 *
 * The nested class binary names and Kotlin data-class bridge methods are part of the
 * runtime contract for already-compiled client mods. Keep those names and signatures
 * stable; see OverlayUiContractTest for the executable contract.
 */
public class OverlayUI {
    public static final OverlayUI INSTANCE = new OverlayUI();
    // debugLogState 的状态转储只在 -Doverlaycompat.debug=true 时输出；gear 隐藏警告不受影响。
    private static final boolean DEBUG_LOG = Boolean.getBoolean("overlaycompat.debug");
    ...
    public static class WindowData {
        ...
        // Kotlin data-class ABI: these component/copy bridge methods must retain their
        // exact names, descriptors, and mask semantics for compiled Kotlin clients.
        ...
        /** Legacy save migration: derives center/size from a pre-constraint-era rect and clears it. */
        static WindowData migrateRect(WindowData v) {
            WindowData next = v.copy();
            Rect rect = next.rect;
            next.center = rect.getCenter(new Vec2());
            next.size = rect.getSize(new Vec2());
            next.rect = null;
            return next;
        }
    }
    ...
    public static class WindowSetting {
        ...
        public void setEnabled(boolean enabled) {
            if (value.getEnabled() == enabled) return;
        ...
        public boolean getPinned() {
            return value.getPinned();
        }

        public void setPinned(boolean pinned) {
            if (value.getPinned() == pinned) return;
            WindowData next = value.copy();
            next.pinned = pinned;
            set(next);
        }
        ...
        private WindowData load() {
            ...
            try {
                ...
                WindowData data = OverlayCompatJson.parse(raw);
        ...
        private void save(WindowData data) {
            ...
            try {
                Core.settings.put(name, OverlayCompatJson.write(data));
            } catch (Throwable t) {
        ...
        public Table buildUI() {
            ...
            pos.update(() -> {
                ...
                sb.append('[')
                    .append(Math.round(d.getCenter().x))
                    .append(',')
                    .append(Math.round(d.getCenter().y))
                    .append(']');
                if (d.getSize() != null) {
                    sb.append('[')
                        .append(Math.round(d.getSize().x))
                        .append('x')
                        .append(Math.round(d.getSize().y))
                        .append(']');
                }
            ...
            ImageButton scale = table.button(Icon.resizeSmall, toggle, Vars.iconSmall, () -> {
                OverlayUI.INSTANCE.showFloatSettingsPanel(panel -> {
                    ...
                    label.update(() -> label.setText(i("scale") + ": " + Strings.fixed(value.getScale(), 1) + "x"));
                    panel.add(label).center().row();

                    Slider slider = new Slider(0.2f, 3f, 0.1f, false);
                    slider.setValue(value.getScale());
                    slider.changed(() -> {
                        WindowData next = value.copy();
                        next.scale = slider.getValue();
                        set(next);
                    });
                    slider.update(() -> slider.setValue(value.getScale()));
                    ...
                    reset.update(() -> reset.setDisabled(Mathf.equal(value.getScale(), 1f)));
                    panel.row();
                });
            }).tooltip(i("scale")).padRight(4f).get();
            scale.update(() -> scale.setChecked(!Mathf.equal(value.getScale(), 1f)));

            table.row();
            if (value.getConstraintX() != null) {
                table.add();
                table.add("X: " + value.getConstraintX().type.name() + " to [" + value.getConstraintX().target + "]").colspan(Math.max(table.getColumns() - 1, 1)).left().row();
            }
            if (value.getConstraintY() != null) {
                table.add();
                table.add("Y: " + value.getConstraintY().type.name() + " to [" + value.getConstraintY().target + "]").colspan(Math.max(table.getColumns() - 1, 1)).left().row();
            }
            return table;
        }
    }
    ...
    public static class Window extends Table {
        ...
        /** Non-resizable windows snap to preferred size; resizable auto-height windows track preferred height. */
        private void applyStableSizing() {
            if (state != WindowState.Stable) return;
            if (!resizable) {
        ...
        /** Stable windows restore their saved center position. */
        private void applyStoredPosition() {
            if (state == WindowState.Stable && data.getValue().getCenter() != null) {
                Vec2 center = data.getValue().getCenter();
                setPosition(center.x, center.y, Align.center);
            }
        }

        /**
         * Snaps the window: stable windows re-apply their saved constraints, dragging windows
         * search for the best live constraints and queue them for the red/yellow guides.
         */
        private void applyAdsorption() {
            adsorption.reset(x, y, width, height);
            if (state == WindowState.Stable) {
                adsorption.applyConstraint(data.getValue().getConstraintX());
                adsorption.applyConstraint(data.getValue().getConstraintY());
            } else {
        ...
        /** Persists the dropped position and the constraints active at drop time. */
        private void commitDragEnd() {
            if (state != WindowState.EndDrag) return;
        ...
        private void updateData() {
            ...
            if (v.getRect() != null) {
                data.set(WindowData.migrateRect(v));
                v = data.getValue();
            }

            if (v.getCenter() == null && parent != null) {
                WindowData next = v.copy();
        ...
        public void rebuild() {
            ...
            if (OverlayUI.INSTANCE.open) {
                ...
                if (data.getValue().getSize() != null) {
                    Vec2 size = data.getValue().getSize();
        ...
        private void applyScale() {
            float scale = data.getValue().getScale();
